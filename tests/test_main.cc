...
namespace gameoflife {
    ...
    TEST_CASE("GameEngine Functionality at corner") {
        ...
        SECTION("Live Cell with 0 neighbors") {
            ...
            std::vector<std::vector<Cell>> test_cells = std::vector<std::vector<Cell>>(
                    test_board.GetGameEngine().GetCells());
            std::vector<std::vector<Cell>> updated_cells = std::vector<std::vector<Cell>>(
                    board.GetGameEngine().GetCells());
