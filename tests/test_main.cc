...
namespace gameoflife{
    ...
    TEST_CASE("GameEngine Functionality at corner"){
        visualizer::BoardUI board(glm::vec2(0,0), 875);
        GameEngine gameEngine;

        std::vector<std::vector<Cell>> cells = std::vector<std::vector<Cell>>(board.GetCells());
        cells[2][2].SetLife(true); //cell in the center, being tested


        SECTION("Live Cell with 0 neighbors"){
            board.SetCells(cells);
            board.SetCells(gameEngine.UpdateCells(board.GetCells()));
            visualizer::BoardUI test_board(glm::vec2(0,0), 875);

            //test_board should have clear board which should be the next step as 0 neighbors kills the cell
            std::vector<std::vector<Cell>> test_cells = std::vector<std::vector<Cell>>(test_board.GetCells());
            std::vector<std::vector<Cell>> updated_cells = std::vector<std::vector<Cell>>(board.GetCells());
        ...
        SECTION("Live Cell with 2 neighbors"){
            ...
            test_board.SetCells(test_cells);
            test_cells = std::vector<std::vector<Cell>>(test_board.GetCells());
